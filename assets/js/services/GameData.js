export default class GameData {
  ...
  static shuffleArray = (array) => array.map(a => ({ sort: Math.random(), value: a })).sort((a, b) => a.sort - b.sort).map(a => a.value);

  static getNextQuestion = () => GameData.questionGame[GameData.index++];

  static setReply(reply, hash) {
    console.log(`You are ${GameData.responseList[hash] == reply ? 'right' : 'wrong'}`)
  }
}